...
Transactional email with pluggable outbound transports (SMTP, Resend, SendGrid, Mailgun, Alibaba DirectMail), `.strav` templates, automatic CSS inlining, and optional Tailwind support. Inbound parsing support is in active development (Postmark webhook shipped; IMAP and SendGrid / Mailgun / SES webhooks pending — see [Inbound mail](#inbound-mail)).
...
| Source | Parser | Authentication |
|---|---|---|
| Postmark Inbound | `PostmarkInboundParser` | Configure at HTTP layer — Postmark does not sign inbound webhooks |

**Pending:**

- IMAP poller (for any mailbox; OAuth2 for Gmail / M365).
- SendGrid Inbound Parse, Mailgun Routes, SES inbound (each with HMAC signature verification).
- `mail.poll(...)` helper for scheduling IMAP polls through `@strav/queue`.

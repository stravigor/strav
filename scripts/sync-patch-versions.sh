...
# Sync all package versions to next patch version with overflow logic
# Usage: ./scripts/sync-patch-versions.sh [--dry-run]
...
# Calculate next version with overflow logic
if [[ $PATCH -eq 99 ]]; then
    if [[ $MINOR -eq 99 ]]; then
        # Both overflow: increment major
        NEW_MAJOR=$((MAJOR + 1))
        NEW_VERSION="${NEW_MAJOR}.0.0"
        echo -e "${YELLOW}Patch and minor overflow detected (99.99 → 0.0), bumping major version${NC}"
        VERSION_TYPE="major"
    else
        # Patch overflow: increment minor
        NEW_MINOR=$((MINOR + 1))
        NEW_VERSION="${MAJOR}.${NEW_MINOR}.0"
        echo -e "${YELLOW}Patch overflow detected (99 → 0), bumping minor version${NC}"
        VERSION_TYPE="minor"
    fi
else
...
case "$VERSION_TYPE" in
    major)
        COMMIT_MSG="chore: bump to major version $NEW_VERSION (patch/minor overflow)

BREAKING CHANGE: Major version bump due to version overflow"
        ;;
    minor)
        COMMIT_MSG="feat: bump to minor version $NEW_VERSION (patch overflow)

New features from accumulated patches"
        ;;
    patch)
        COMMIT_MSG="fix: bump to patch version $NEW_VERSION
...
BREAKING CHANGE: Major version bump due to patch/minor overflow
Updated all package submodules to version $NEW_VERSION."
        ;;
    minor)
        WORKSPACE_MSG="feat: update package submodules to v$NEW_VERSION

Minor version bump due to patch overflow.
Updated all package submodules to version $NEW_VERSION."
        ;;
    patch)
        WORKSPACE_MSG="fix: update package submodules to v$NEW_VERSION

Updated all package submodules to patch version $NEW_VERSION."
        ;;
esac
